...
from app.schemas.report import ReportCreate, ReportUpdate, Report, ReportGenerationRequest, ReportGenerationStatus
...
async def generate_report_background(
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            logger.info(f"Starting report generation for report_id={report_id}, topic={topic}")
            ...
            logger.error(f"Report generation failed for report_id={report_id}: {str(e)}")
...
@router.post("/generate", response_model=ReportGenerationStatus, status_code=status.HTTP_202_ACCEPTED)
async def generate_report(
    ...
    report_create = ReportCreate(
        ...
        description=f"Generated report on {request.topic}",
    ...
    logger.info(f"Report generation queued: report_id={report.id}, user={current_user.id}")
