...
async def export_patents_csv(
    ...
    writer.writerow([
        ...
        'Assignee', 'Inventors', 'Filing Date', 'Created At', 'Updated At'
    ])
...
async def export_patents_excel(
    db: AsyncSession = Depends(get_db)
):
    patents = await patent_crud.get_all(db, skip=0, limit=10000)
    ...
    for row, patent in enumerate(patents, start=1):
        ...
        worksheet.write(row, 6, patent.filing_date.strftime('%Y-%m-%d') if patent.filing_date else '')
        worksheet.write(row, 7, patent.created_at.strftime('%Y-%m-%d %H:%M:%S') if patent.created_at else '')
...
async def export_patents_pdf(
    db: AsyncSession = Depends(get_db)
):
    patents = await patent_crud.get_all(db, skip=0, limit=100)
    ...
    for patent in patent_schemas:
        ...
        story.append(Paragraph(f"Assignee: {patent.assignee or 'Not specified'}", normal_style))
...
async def export_reports_csv(
    ...
    result = await db.execute(select(ReportModel))
    ...
    writer.writerow([
        'Report ID', 'Topic', 'Report Type', 'Patent Count', 'Created At'
    ])
    
    for report in reports:
        patent_count = len(report.patent_ids) if report.patent_ids else 0
        writer.writerow([
            report.id,
            report.topic,
            report.report_type,
...
async def export_reports_pdf(
    ...
    for report in reports:
        ...
        story.append(Paragraph(report.content, normal_style))
        story.append(Spacer(1, 12))
...
async def export_custom_data(
    ...
    if data_type == 'patents':
        ...
        search_params = PatentSearch(
            ...
            filing_date_from=datetime.fromisoformat(filters['filing_date_from']) if filters.get('filing_date_from') else None,
            filing_date_to=datetime.fromisoformat(filters['filing_date_to']) if filters.get('filing_date_to') else None
        )
        
        patents, _ = await patent_crud.advanced_search(db, search_params)
        patent_schemas = [Patent.model_validate(patent) for patent in patents]
