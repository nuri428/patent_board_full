...
from app.models import PatentModel, ReportModel
...
async def get_analytics_overview(
    ...
    patents_last_30 = await db.execute(
        select(func.count(PatentModel.patent_id))
    ...
    patents_last_year = await db.execute(
        select(func.count(PatentModel.patent_id))
        .where(PatentModel.created_at >= last_year)
    )
...
async def get_patents_timeline(
    db: AsyncSession = Depends(get_db),
    days: int = 365
):
...
async def get_patents_by_month(
    db: AsyncSession = Depends(get_db),
    months: int = 12
):
...
async def get_patents_by_status(
    db: AsyncSession = Depends(get_db)
):
    status_query = (
        ...
        "status_distribution": [
            {
                "status": row[0],
                "count": row[1],
                "percentage": round((row[1] / total_count * 100), 2) if total_count > 0 else 0
            }
...
async def get_reports_analytics(
    ...
    report_types_query = (
        select(ReportModel.report_type, func.count(ReportModel.id))
