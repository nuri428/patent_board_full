...
class AnalyticsCRUD:
    ...
    async def get_user_analytics(self, user_id: int) -> UserAnalytics:
        ...
        searches_result = await self.db.execute(
            select(func.count(SearchQuery.id)).where(SearchQuery.user_id == user_id)
        )
        ...
        avg_time_result = await self.db.execute(
            select(func.avg(SearchQuery.execution_time_ms)).where(
    ...
    async def get_patent_analytics(self, patent_id: int) -> PatentAnalytics:
        ...
        patent = await self.db.execute(select(Patent).where(Patent.id == patent_id))
    ...
    async def get_top_searches(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [{"query": row.query_text, "count": row.count} for row in result]
