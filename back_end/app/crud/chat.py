...
from back_end.app.models import ChatSession, ChatMessage
from back_end.app.schemas import ChatSessionCreate, ChatMessageCreate
import json
import uuid
...
class ChatCRUD:
    ...
    async def add_message(
        ...
        await self.db.execute(
            select(ChatSession)
            .where(ChatSession.id == message_create.session_id)
        )
        ...
        db_message = ChatMessage(
            ...
            patent_references=json.dumps(message_create.patent_references) if message_create.patent_references else None,
    ...
    async def count_session_messages(
        ...
        query = select(func.count(ChatMessage.id)).where(ChatMessage.session_id == session_id)
        if message_type:
            query = query.where(ChatMessage.message_type == message_type)
